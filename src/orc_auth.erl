...
-record(orc_auth, { token, user, email, active, paths = [] }).
...
auth(Request = #request{ path = Path, headers = Headers }) ->
	...
	case proplists:get_value(<<"Authorization">>, Headers ) of
		undefined ->
			...
			#response{ status = 401, headers = [{  <<"WWW-Authenticate">>,<<"Basic realm=\"orc\"">> }, {<<"Content-Length">>, <<"0">> }]};
		Authorization ->
			F = fun() ->
				{ ok, Salt } = application:get_env(orc,salt),
...
add(User,Email,Password) ->
	...
	F = fun() ->
		remove(User,Email),
		ok = mnesia:write(#orc_auth{ token = Token, user = User, email = Email, active = true, paths = [] }),
...
remove(User,Email) ->
	F = fun() ->
		case mnesia:match_object(#orc_auth{ user = User, email = Email, token = '_', active = '_', paths = '_' }) of
			[] -> ok;
			Records ->
...
grant(User,Pattern) ->
	F = fun() ->
		case mnesia:match_object(#orc_auth{ user = User, email = '_', token = '_', active = true, paths = '_' }) of
			[] -> ok;
			[ Auth = #orc_auth{ paths = Paths } ] ->
				ok = mnesia:write(Auth#orc_auth{ paths = [ Pattern | Paths ]}),
...
revoke(User,Pattern) ->
	F = fun() ->
		case mnesia:match_object(#orc_auth{ user = User, email = '_', token = '_', active = true, paths = '_' }) of
			[] -> ok;
			[ Auth = #orc_auth{ paths = Paths }] ->
				ok = mnesia:write(Auth#orc_auth{ paths = lists:delete(Pattern,Paths) }),
...
install(Nodes) ->
	{ atomic, ok } = mnesia:create_table(orc_auth, [
		{ attributes, record_info(fields,orc_auth) },
		{ disc_copies, Nodes }]).
