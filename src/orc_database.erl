...
-record(orc_database, { nodes, tables }).
...
initialize(Nodes) ->
	...
	{ ok, Tables } =  application:get_env(orc,tables),	% the tables contains a list of all modules with install functions
...
init(Nodes) ->
	...
	{ ok, Tables } =  application:get_env(orc,tables),
	{ ok, Timeout } = application:get_env(orc,table_timeout),
	rpc:multicall(Nodes,mnesia,start,[]),
	case mnesia:wait_for_tables(Tables,Timeout) of
		ok ->
			error_logger:info_msg("Database started on ~p", [ Nodes ]),
			{ ok, #orc_database{ nodes = Nodes, tables = Tables }};
		{ timeout, Remaining } ->
...
terminate(Reason,#orc_database{ nodes = Nodes } = State) ->
